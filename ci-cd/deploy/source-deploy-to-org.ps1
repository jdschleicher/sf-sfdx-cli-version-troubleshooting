...
if ($deploy_path_directory_exists) {
    ...
    run_salesforce_deployment_by_packagexml -alias $alias -package_xml_path $package_xml_path

    # Destructive Package XML
    $destructive_package_xml_path = "$deploy_path/destructiveChanges/destructiveChanges.xml"
    run_salesforce_destructive_changes -alias $alias -deploy_path $deploy_path -destructive_package_xml_path $destructive_package_xml_path    
} else {
